// === animations.js - Consolidated ===
// Generated from: animations.js, scroll-animations.js
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function handleMenuScroll() {
        if (!menuSuperior) return;
        if (window.scrollY > 50) {
            menuSuperior.classList.add('scrolled');
            if (navegacionRapida) {
                navegacionRapida.style.top = '55px';
            }
        } else {
            menuSuperior.classList.remove('scrolled');
            if (navegacionRapida) {
                navegacionRapida.style.top = '70px';
            }
        }
    }
